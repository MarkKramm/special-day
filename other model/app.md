...
const NOTE =
  "I hope the year ahead gives back some of the kindness you quietly bring to others.";
...
function Sky({ closing = false }: { closing?: boolean }) {
  return (
...
function Cake({ lit, progress, out }: { lit: boolean; progress: number; out: boolean }) {
  ...
  return (
    <div className="cake" aria-hidden="true">
      ...
      {/* Painted last so the wick and flame always layer above the cake. */}
...
function TapProgress({ taps, hidden }: { taps: number; hidden: boolean }) {
  return (
    <div className={`tap-progress ${hidden ? "is-hidden" : ""}`} aria-hidden="true">
      <span className="tap-progress-glow" style={{ width: `${(taps / REVEAL_TAPS) * 100}%` }} />
      {Array.from({ length: REVEAL_TAPS }, (_, index) => (
        <span className={`tap-progress-dot ${index < taps ? "is-lit" : ""}`} key={index} />
      ))}
    </div>
  );
}
...
function MessageCard({ onClose, closing }: { onClose: () => void; closing: boolean }) {
  ...
  return (
    <div className={`note-backdrop ${closing ? "note-is-closing" : ""}`} role="presentation">
      <article
    ...
    >
      ...
      {scene === "reveal" && (
        ...
        >
          ...
          <TapProgress taps={tapCount} hidden={revealComplete} />
      ...
      {scene === "closing" && (
        <section className="scene closing-scene scene-enter" aria-label="A final birthday wish">
          <svg className="final-smoke" viewBox="0 0 390 800" preserveAspectRatio="none" aria-hidden="true">
            <path d="M142 690C123 627 188 602 161 541C137 486 206 459 185 405C164 351 231 315 216 267C207 236 236 216 260 190" />
          </svg>
          <span className="new-star" aria-hidden="true"><i /></span>
